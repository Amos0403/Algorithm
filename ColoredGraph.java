 ...
 * 
 * @author Amos Le Due Date: 12/10/2023
 */
...
public class ColoredGraph {
	...
	*/
	public int[] getSolution(int start, int end) {
		...
		String[] colorOrder = {"red", "yellow", "blue"};
		...
		int[] distance = new int[this.getGraphSize()];
	
		// Create a predecessor array to store the path
		int[] pred = new int[this.getGraphSize()];
		...
		while (!queue.isEmpty()) {
			...
			for (Edge edge : node.getEdges()) {
				GraphNode nextNode = edge.getOther(node.getData());
	
				// If this adjacent node is the destination node, construct the path and return it
				if (nextNode.getData() == end && edge.getColor().equals(colorOrder[distance[node.getData()] % 3])) {
					int[] path = new int[distance[node.getData()] + 1];
					int curr = end;
				...
				// Else, if the adjacent node has not been visited and its edge color is the next color in the order, then mark it visited, enqueue it, and update its predecessor
				if (!visited[nextNode.getData()] && edge.getColor().equals(colorOrder[distance[node.getData()] % 3])) {
					visited[nextNode.getData()] = true;
					queue.add(nextNode);
					distance[nextNode.getData()] = distance[node.getData()] + 1;
					pred[nextNode.getData()] = node.getData();
				}
			}
		}
